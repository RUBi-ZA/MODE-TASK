...
def getVector(vtMatrix, mode, direction, normalise=True, eps=0.02):
    try:
        f = open(vtMatrix, 'r')
        vlines = f.readlines()
        f.close
    ...
    for v in vectorFile:
        ...
        if normalise:
            ...
            mag = 1 # no normalisation
        for e in v:
            e = direction * (e/mag) + eps # eps is only added to enhance arrow visibility
            unitVec.append(e)
...
def visualise(pdb_lines, atomT, mode, vectors, outdir, head, tail, arrowLength,
              coloursSpecified, chainSpecified, aUnits):
    if aUnits[0] == 'all':
        structure = 'VISUAL'
    ...
    if coloursSpecified == 'none':
        colours = ['red', 'blue', 'ochre', 'purple', 'yellow', 'red', 'cyan',
                   'pink', 'silver', 'violet', 'ochre', 'blue2', 'cyan2', 'iceblue',
                   'lime', 'green2', 'green3', 'violet', 'violet2', 'mauve']
    else:
...
def main(args):
    ...
    vectorFile = getVector(vtMatrix, mode, direction, args.normalise_arrows)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--atomType",
                        ...
                        default='X')
    # additional opitions
    parser.add_argument("--head", help="Specify radius radius of arrow head. Default = 2.20",
    ...
    parser.add_argument("--normalise_arrows",
                        help="Normalise the arrow lengths", action="store_true",
                        default=False)
    parser.add_argument("--arrowLength",
                        help="Increase of decrease arrow length by a specified factor",
                        type=float, default=9.0)
    parser.add_argument("--colourByChain", default='none', type=str,
                        help="Enter a comma separated list of colours for each chain.\nEg: If you have four chains in your complex then enter something like: red,blue,green,pink\nThis will colour the following:\nChain 1 as red\nChain 2 as blue\nChain 3 as green\nChain 4 as pink\nNote that these colours must match available colours in VMD\n")
    parser.add_argument("--chain", help="Draw arrows for specified chain only",
                        default='all')
